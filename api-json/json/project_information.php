...
class project_information
{
    ...
    public function EditDemand():string{
        ...
        $verification   =   [
            ...
            [$data['user'],code::PARAMETER_ERROR,'required'],
        ];
        (new check())->set_code($verification);
        $key_id =   Get_data_rsa($key_id);
        $details_demand    =   Get_details_where(['project_id' =>  $key_id],'*','t_project_demand');
        ...
        updata_detail($code, 't_project_demand', $details_demand['id']);
        return success($code,code::REQUEST_SUCCESSFUL);
    }
...
//                'hotel'  =>  Get_details_value($details_day['hotel'],'title','t_resources_hotel'),
    ...
    {
        $sql = "INSERT INTO `t_project_demand`( `project_id`) VALUES ( ".$key_id.");";
        $add_id =   $this->mysql->query($sql);
        if(empty($add_id)){
            throw new Exception(code::SYSTEM_ERROR);
        }
