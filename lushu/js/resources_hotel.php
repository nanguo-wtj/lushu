<script>
    const vm = new Vue({
        ...
        methods: {
            ...
            post_poi(){
                this.project_data.policy    =   $('#policy').html();
                console.log(this.project_data)
                const url   =   "<?=$_Post_url?>?cmd=resource&list=hotel_add";
                post_url(url,this.project_data,true).then(res => {
                    over_Time_url('./resources_hotel_details.html?key_id='+res.data.key_id,500);
                },error=>{
            ...
            GetResourcesList:function () {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=hotel";
            ...
            GetListNumber:function(e){
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=hotel_number";
            ...
            get_details(e){
                Jump_url('/lushu/resources_hotel_details.html?key_id='+e);
            ...
            Set_search_rating:function (e) {
                var _that   =   this;
                _that.resources_key.rating = e;
                if(e == 0){
                    _that.resources_key.rating_value    =   '全部';
                }else {
                    _that.resources_key.rating_value    =   e+'星';
                }
                _that.search();
                $('#rating').hide()
            },
            ...
            search_rating_show:function (){
                event.stopPropagation();
                if ($("#rating").is(":visible")) { //判断元素是否可见
                    $("#rating").hide(); //如果可见则隐藏
                } else {
                    $("#rating").show(); //如果不可见则显示
                }
            },
