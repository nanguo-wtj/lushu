    ...
    new Vue({
        ...
        methods: {
            ...
            add_content:function (e,b) {
                // var _that   =   this;
                // if(b == 2){
                //     window.editor.setHtml(e.title)
                // }else {
                //     window.editor.setHtml("<img src='"+e.url+"' > ")
                // }
                console.log('e',e);
                setTimeout(() => {
                    this.$forceUpdate()
            ...
            GetTripList:function (e) {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=trip";
                if(e){
                    _that.search_Trip_data.page = e;
                }
                _that.search_Trip_data.key_id = _that.key_id;
                post_url(url,_that.search_Trip_data,false,true).then(res => {
                    if(res.data.list.length < 1){
                        if(_that.search_Trip_data.page > 1){
                            _that.search_Trip_data.page--;
                        }
                        _that.search_Trip_data.status = false;
                        return  PopUpPrompt('前面已经没有了！',7);

                    }else {
                        _that.search_Trip_data.status = true;
                        _that.list.trip_list = [];
                        res.data.list.forEach(function (item, index, arr) {
                            var str = {
                                'id': item.id,
                                'url': item.picture,
                            ...
                            if (_that.data.Highlights_id.indexOf(item.id) >= 0) {
                                str.status = true;
                            }
                            _that.list.trip_list.push(str);
                        });
                    }
                },error=>{
                });
            },
            add_project_trip:function (e) {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=export&list=add_trip";
                post_url(url,{key_id:_that.key_id,trip_id:e.id},true).then(res => {
                    e.status = true;
                    var str =   {
                            'id'        :   e.id,
                            'url'       :   e.url,
                    ...
                    _that.data.Highlights_list.push(str)
                },error=>{

                });
            },
            del_project_trip:function (e,c) {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=export&list=del_trip";
                post_url(url,{key_id:_that.key_id,trip_id:e.id},true).then(res => {
                    if(c){
                        _that.list.trip_list.forEach(function(item, index, arr) {
                            if(item.id == e.id){
                                item.status = false;
                            }
                        });
                    }
                        _that.data.Highlights_list.forEach(function(item, index, arr) {
                            if(item.id == e.id){
                                _that.data.Highlights_list.splice(index, 1);
                            }
                        });
            ...
            add_project_note:function (e) {
                console.log(e)
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=export&list=add_note";
            ...
            del_project_note:function (e,c) {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=export&list=del_note";
                post_url(url,{key_id:_that.key_id,note_id:e.id},true).then(res => {
                    if(c){
            ...
            get_project_city_list:function () {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=project_city_export";
                post_url(url,{key_id:_that.key_id},false).then(res => {
            ...
            Post_content:function (){
                ...
                const url   =   "<?=$_Post_url?>?cmd=export&list=add_project_content";
            ...
            previousPageTrip:function () {
                if(this.search_Trip_data.page < 2){
                    return false;
                }
                this.search_Trip_data.page--;
                this.search_Trip_data.status = true;
                this.GetTripList();
            },
            nextPageTrip:function () {
                if(this.search_Trip_data.status == false){
                    return false;
                }
                this.search_Trip_data.page++;
            ...
            OpenItinerary:function(){
                $('.XCJS').css('display','block')
                this.search_Itinerary_data = {title:'',page:1,status:true};
                this.Get_Seach_Itinerary();
            },
            Get_Seach_Itinerary:function () {
                var _that = this;
                if(_that.list.note_status == true){
                    _that.get_content_Itinerary();
                }else if(_that.list.img_status == true){
                    _that.get_img_Itinerary();
                }
            },
            get_content_Itinerary:function (e) {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=note";
                _that.search_Itinerary_data.key_id = _that.key_id;
                if(e){
                    _that.search_Itinerary_data.page = e;
                }
                post_url(url,_that.search_Itinerary_data,false,true).then(res => {

                    if(res.data.list.length < 1){
                        if(_that.search_Itinerary_data.page > 1){
                            _that.search_Itinerary_data.page--;
                        }
                        _that.search_Itinerary_data.status = false;
                        return  PopUpPrompt('暂无更多！',7);
                    }else {
                        _that.search_Itinerary_data.status = true;
                        _that.list.note_list  =   [];
                        res.data.list.forEach(function(item, index, arr) {
                            var str =     {
                                'id'        :   item.id,
            ...
            get_img_Itinerary:function () {
                var _that   =   this;
                const url   =   "<?=$_Post_url?>?cmd=resource_list&list=picture";
                _that.search_Itinerary_data.key_id = _that.key_id;

                post_url(url,_that.search_Itinerary_data,false).then(res => {
